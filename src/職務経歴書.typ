...
#v(10pt)
#section[職務要約]
研究開発組織のデータ活用と機械学習ワークフローを支える基盤開発に従事してきたソフトウェアエンジニア。既存のデータ管理を、探索、取り込み、実験追跡、モデル成果物管理、本番推論まで一貫して扱える社内プラットフォームへ発展させ、研究者が実験結果を比較・再現・共有しやすい環境を整備した。データ準備時間の短縮、処理スループットの向上、手作業に依存しない運用体制の構築を通じて、研究開発サイクルの高速化に貢献。医学・バイオインフォマティクス・科学計算の知識を背景に、技術実装と研究現場の課題をつなぎ、継続的に使われる内部基盤を設計・改善できる点を強みとする。
...
#section[自己PR]
研究開発組織で継続的に使われる基盤には、機能を実装するだけでなく、データ量の増加、実験の再現性、運用のしやすさを見据えた設計が必要だと考えています。MOLCUREでは、研究者が必要なデータを探し、取り込み、分析し、実験結果を追跡できるデータ・AIプラットフォームの設計と実装を担当しました。これにより、属人的になりやすいデータ準備や実験管理を共通の運用フローにまとめ、分析と機械学習の双方で再利用しやすい環境を整備しました。

バックエンド開発では、研究者が日常的に使うデータ探索、メタデータ管理、実験追跡、モデル成果物閲覧、GPU推論ジョブ管理の機能を設計・運用しました。データ検出からテーブル生成、実験結果の記録、推論ジョブの実行までを自動化し、データと成果物を比較・再現・共有しやすい状態に整理しました。認証・認可、キャッシュ、テスト、可観測性の整備にも取り組み、継続利用に耐える信頼性と応答性の改善を進めました。

インフラと運用面では、環境構築、アクセス制御、ネットワーク設定、リリース手順をコード化・自動化し、変更作業の再現性とレビュー容易性を高めました。さらに、次世代シーケンシングデータ処理パイプラインでは、データ準備時間を日単位から時間単位へ短縮し、処理スループットを30倍向上させました。医学・バイオインフォマティクス・科学計算の知識を背景に、研究現場の課題を理解しながら、要件整理、データモデル設計、API開発、インフラ自動化、運用改善まで横断して推進できる点が強みです。
